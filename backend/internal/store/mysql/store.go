...
func (s *MySQLStore) ResetSeedData(ctx context.Context) (int64, int64, error) {
...
func (s *MySQLStore) Migrate(ctx context.Context) error {
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS categories (
			...
			seed_key    VARCHAR(255) NOT NULL DEFAULT '',
		...
		`CREATE TABLE IF NOT EXISTS formulas (
			...
			CONSTRAINT fk_formulas_domain_category FOREIGN KEY (domain) REFERENCES categories(slug) ON UPDATE RESTRICT ON DELETE RESTRICT
		)`,
		...
		`CREATE TABLE IF NOT EXISTS lookup_tables (
			...
			created_at VARCHAR(35)  NOT NULL,
			updated_at VARCHAR(35)  NOT NULL
			,CONSTRAINT fk_lookup_tables_domain_category FOREIGN KEY (domain) REFERENCES categories(slug) ON UPDATE RESTRICT ON DELETE RESTRICT
		)`,
	...
	// MySQL DDL auto-commits each statement, so this path is explicitly
	// best-effort idempotent instead of pretending rollback can undo a
	...
	// Idempotent column additions for old databases. MySQL 8 has no
	// IF NOT EXISTS on ADD COLUMN, so we tolerate error 1060
	// ("Duplicate column name") which means the column was added by
	// a previous run.
	alters := []string{
	...
	for _, stmt := range alters {
		if _, err := s.db.ExecContext(ctx, stmt); err != nil {
			...
			if errors.As(err, &myErr) && (myErr.Number == 1060 || myErr.Number == 1061 || myErr.Number == 1826) {
				...
				// any of these means a prior migration already
				// applied this change.
				continue
			}
...
func (r *formulaRepo) CreateWithInitialVersion(ctx context.Context, f *domain.Formula, v *domain.FormulaVersion) error {
	...
	if _, err := tx.ExecContext(ctx,
		`INSERT INTO formulas (id, name, domain, description, created_by, created_at, updated_at, seed_key) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
...
// formulaSortColumns mirrors the sqlite/postgres whitelists. The handler
// validates the public sortBy field; this map is the only place that
// translates it to a SQL column expression. Never concatenate raw user
// input into the ORDER BY.
...
func (r *formulaRepo) List(ctx context.Context, filter domain.FormulaFilter) ([]*domain.Formula, int, error) {
	...
	// MySQL has no NULLS LAST clause, so emulate it with the
	// `(col IS NULL)` trick: NULL rows always sort to the end
	// regardless of asc/desc on the actual column.
