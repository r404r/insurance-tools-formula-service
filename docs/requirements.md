...
### 2.1b Formula CRUD Workflow (公式管理工作流)

- **FR-007**: 公式列表页展示所有公式，支持按领域(life/property/auto)过滤和关键字搜索
- **FR-008**: Admin/Editor角色可通过弹窗创建新公式（指定名称、领域、描述），创建后自动跳转编辑器
- **FR-009**: 可视化编辑器中，拖拽节点到画布后点击「保存」自动创建第一个版本(Draft)；后续保存创建递增版本
- **FR-009a**: 保存后显示「已保存」反馈，并自动刷新版本号显示
- **FR-009b**: 编辑器测试面板可输入JSON参数，调用计算API并即时显示结果
