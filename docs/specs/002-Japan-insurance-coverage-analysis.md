...
| **8** | **チェインラダー法 LDF** | **三角形矩阵迭代** | ✅ **已支持**（task #040） | TableAggregate 节点 + 预计算 ratio 列 |
...
### 🥈 优先级 2 — 表的 SQL 风格 Aggregate 节点（filter + sum/avg/count）✅ 已实现 task #040

**收益**：链梯法（公式 8）从噩梦变成两行公式；任何"对历史数据做带条件聚合"的场景一并解锁
**工作量**：~3 天
**详细设计**：见 [`004-table-aggregate-node.md`](./004-table-aggregate-node.md)
**已实现**：[task #040](../tasks/040-table-aggregate-node.md)，公式 8 的支持等级从 ⚠️ 升级为 ✅。`TestTableAggregate_ChainLadderLDF` 用 spec 004 §2.2 的真实三角形数据验证了 LDF₁/LDF₂/LDF₃ 都能用一个 TableAggregate 节点 + 一个 dynamic input filter 表达。
...
| 指标 | 值（最新） |
|---|---|
| **20 条中纯原生支持** | 19（task #040 后公式 8 升级；task #039 后公式 19 升级） |
| **20 条中需要 workaround** | 1（公式 14：标准正态分位数硬编码） |
| **20 条中 UX 缺陷但语义正确** | 0 |
| **20 条中完全不可实现** | 0 |
