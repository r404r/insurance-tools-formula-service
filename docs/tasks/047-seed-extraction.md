...
- [x] 2.1 Runner 加 `--dry-run` flag（不联系 backend，全量校验占位符闭合）
- [x] 2.2 Runner 加 `--only NAME` flag（live 模式下自动检测 dry-run id 残留并报错）
- [x] 2.2a 加单元测试：`TestRun_DryRunResolvesAllPlaceholders`、`TestRun_DryRunFailsWhenPlaceholderUnresolved`、`TestRun_OnlyFiltersDependencies_DryRun`
- [ ] 2.3 ~~Bundle 文件按 domain 分子目录~~（推迟：数字前缀已经提供 git-friendly 顺序，分子目录会引起一次大 churn 但收益边际，留作未来需要时再做）
- [x] 2.4 `Dockerfile.seed` + `docker-compose.yml` `seed` profile 集成 + 端到端测试通过
- [x] 2.5 项目 README 更新（Default Account & Seed Data 章节 + Loading the seed bundles 子节 + Project Structure）
- [x] 2.6 提交阶段 2 commit（含 2 轮 codex review：round 1 3 P2/P3 → round 2 LGTM）
