...
- **TableAggregate ノード**（参照テーブルに対する SQL 風集約、task #040 で追加）：テキスト文法にまだ SQL 集約の構文がありません。プリセットの `日本損害保険 チェインラダー LDF` 計算式が初の利用例で、テキストモードに切り替えると明示的なエラーが表示され、ビジュアルエディタへ案内されます。
